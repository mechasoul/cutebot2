...
public class UdpQueueManager extends NativeResourceHolder {
   ...
   * @return True if adding the packet to the queue succeeded
   */
  public boolean queuePacketWithSocket(long key, ByteBuffer packet, InetSocketAddress address, int fileDescriptor) {
    synchronized (library) {
      if (released) {
        return false;
      }
      ...
      int port = address.getPort();
      String hostAddress = address.getAddress().getHostAddress();
      return library.queuePacketWithSocket(instance, key, hostAddress, port, packetBuffer, length, fileDescriptor);
    }
  }
